...
public:
    ...
    config_t cfg;
    Config1(QString name){
        database=new FileDataBase(name);
    }
    ~Config1(){}
    void load_database()
    {
        QByteArray b=p_database->get();
       // cfg=decode_from_json(b);
        cfg=json2struct(b,WholeSetting);
    }
    void *json2struct(QByteArray json_src,int kind)
    {
        void *d=NULL;
        switch(kind){
        case WholeSetting:
      ...
      QByteArray struct2json(void *dat,int kind)
      {
          QByteArray ba;
          ba.clear();
          switch(kind){
